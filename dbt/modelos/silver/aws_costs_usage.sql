...
, aws AS (
	...
	WHERE
		NOT (COALESCE(aws.billing_origin, '') = 'AWS Marketplace' AND COALESCE(aws.aws_product_name, '') NOT LIKE '%Bedrock%')
		AND COALESCE(aws.aws_product_name, '') <> 'OCBAWS Brazil 2P'
		AND COALESCE(aws.description, '') <> 'Contractual Credit - Credits, credit from account: 887592687927' -- (-) aws_costs_marketplace
		...
		AND COALESCE(aws.circle_id, '999999999999999999999999') <> '999999999999999999999999' -- (-) aws_costs_untagged
		--AND COALESCE(aws.resource_id, '') <> '' -- (-) aws_costs_shared
		--AND COALESCE(aws.circle_id, '999999999999999999999999') <> '999999999999999999999999' -- (-) aws_costs_untagged
)
